...
- Branch: `claude/ios-voice-keyboard-oBcac`. Both targets compiled clean against the
  iphonesimulator SDK as of the last session.
- Device install blocker was the missing iOS device-support platform component in
  Xcode 26.2 (`xcodebuild` couldn't resolve a device destination). Resolve via Xcode
  UI (it fetches the targeted device support) or `xcodebuild -downloadPlatform iOS`
  (8.4GB, includes the sim runtime).
